...
public class ChordInterval {
    ...
    public static boolean isInInterval(byte[] target, byte[] start, byte[] end, boolean isStartOpen, boolean isEndClosed) {
        final ChordInterval interval = new ChordInterval(IdentifierUtils.toNumerical(start), IdentifierUtils.toNumerical(end), isStartOpen, isEndClosed);
        return interval.contains(IdentifierUtils.toNumerical(target));
    }
